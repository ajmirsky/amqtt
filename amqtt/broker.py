...
class Broker:
    ...
    async def start(self) -> None:
        ...
        await self.plugins_manager.fire_event(str(EventBroker.PRE_START))
        try:
            await self._start_listeners()
            self.transitions.starting_success()
            await self.plugins_manager.fire_event(str(EventBroker.POST_START))
    ...
    async def shutdown(self) -> None:
        ...
        await self.plugins_manager.fire_event(str(EventBroker.PRE_SHUTDOWN))
        ...
        await self.plugins_manager.fire_event(str(EventBroker.POST_SHUTDOWN))
    ...
    async def _handle_client_session(
        ...
        await self.plugins_manager.fire_event(str(EventBroker.CLIENT_CONNECTED), client_id=client_session.client_id)
    ...
    async def _handle_disconnect(
        ...
        if result is None:
            ...
            await self.plugins_manager.fire_event(str(EventBroker.CLIENT_DISCONNECTED), client_id=client_session.client_id)
            return False
        return True
        ...
        for index, subscription in enumerate(subscriptions.topics):
            if return_codes[index] != AMQTT_MAGIC_VALUE_RET_SUBSCRIBED:
                await self.plugins_manager.fire_event(
                    str(EventBroker.CLIENT_SUBSCRIBED),
    ...
    async def _handle_unsubscription(
        ...
        for topic in unsubscription.topics:
            ...
            await self.plugins_manager.fire_event(
                str(EventBroker.CLIENT_UNSUBSCRIBED),
                client_id=client_session.client_id,
                topic=topic,
            )
    ...
    async def _handle_message_delivery(
        ...
        if not permitted:
            ...
            await self.plugins_manager.fire_event(
                str(EventBroker.MESSAGE_RECEIVED),
                client_id=client_session.client_id,
                message=app_message,
            )
