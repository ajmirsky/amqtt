...
from amqtt.mqtt.constants import PUBREC, SUCCESS
...
class PubrecVariableHeader(MQTTVariableHeader):
    """Variable header for PUBREC packet."""
    ...
    def __repr__(self) -> str:
        return f"PubrecVariableHeader(packet_id={self.packet_id}, reason_code={self.reason_code}, properties={self.properties})"
...
class PubrecPacket(MQTTPacket[PubrecVariableHeader, None, MQTTFixedHeader]):
    """PUBREC packet."""
    ...
    def __init__(
        ...
        variable_header: PubrecVariableHeader = None,
        payload=None,
    ) -> None:
        if fixed is None:
            fixed = MQTTFixedHeader(PUBREC, 0x00)
    ...
    def packet_id(self, packet_id: int) -> None:
        ...
        if self.variable_header is None:
            self.variable_header = PubrecVariableHeader()
    ...
    def build(cls, packet_id: int) -> Self:
        """Build a PUBREC packet for MQTT 3.1.1."""
        variable_header = PubrecVariableHeader(packet_id)
    ...
    def build_mqtt5(cls, packet_id: int, reason_code: int = SUCCESS, properties: Properties = None) -> Self:
        """Build a PUBREC packet for MQTT5."""
        variable_header = PubrecVariableHeader(packet_id, reason_code)
