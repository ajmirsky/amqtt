...
from amqtt.contrib.auth_db.plugin import AuthUserDBPlugin, AuthTopicDBPlugin
...
# ######################################
# Tests for the AuthUserDBPlugin
...
async def test_db_auth(db_connection, user_manager, user_pwd, session_pwd, outcome):
    ...
    broker_context.config = AuthUserDBPlugin.Config(
        connection=db_connection
    )
    db_auth_plugin = AuthUserDBPlugin(context=broker_context)
...
# ######################################
# Tests for the AuthTopicDBPlugin
...
async def test_topic_publish_filter_plugin(db_file, topic_manager, db_connection, acl_topic, msg_topic, outcome):
    ...
    broker_context = BrokerContext(broker=Broker())
    broker_context.config = AuthTopicDBPlugin.Config(
        connection=db_connection
    )
    db_auth_plugin = AuthTopicDBPlugin(context=broker_context)
...
async def test_topic_subscribe(db_file, topic_manager, db_connection):
    ...
    sub_topic_list = await topic_manager.add_topic(client_id, '$SYS/#', Action.SUBSCRIBE)
